...
from pylearn2.expr import activations

class NNSAE(DenoisingAutoencoder):
    """
    ...
    * Adds support for adaptable learning rate based on sparsity of activations
    """
    def __init__(self, corruptor, nvis, nhid, act_enc, act_dec, 
        ...
        super(NNSAE, self).__init__(corruptor, nvis, nhid, act_enc, act_dec, 
        ...
        if isinstance(self.act_enc, ScaledLogistic):
            ...
            self.lrateIP = sharedX(self.act_enc.lrateIP)
            self.meanIP = self.act_enc.meanIP
...
class ScaledLogistic(object):
    def __init__(self, n, lrateIP=0.001, meanIP=0.2, init_a=1, init_b=-3):
...
class NNSAEWeightDecay(object):
    def __init__(self, model):
        ...
        if not model.tied_weights:
            update_weights.append('Wprime') 
        for p in model.get_params():
            if p.name in update_weights:
