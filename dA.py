...
class dA(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, input=None,
                 ...
                 W=None, bhid=None, bvis=None, nl=True, mult=None):
        """
        ...
        if not W:
            ...
            #initial_W = numpy.asarray(numpy_rng.normal(
            #          loc=0,
            #          scale=0.1**0.5,
            #          size=(n_visible, n_hidden)), dtype=theano.config.floatX)
            W = theano.shared(value=initial_W, name='W', borrow=True)

        # added name to b_prime variable
        if not bvis:
        ...
        if mult is None:
            self.mult = theano.shared(value=numpy.ones(
                    n_hidden,
                    dtype=theano.config.floatX), 
                                      borrow=True)
        else:
    ...
    def get_hidden_values(self, input):
        ...
        return self.neuron_func((T.dot(input, self.W) + self.b) * self.mult)
    ...
    def get_reconstructed_input(self, hidden):
        ...
        """
        return  self.neuron_func(T.dot(hidden, self.W_prime) + self.b_prime)
